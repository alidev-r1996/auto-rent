...
import CarCommentForm from "@/app/(main)/cars/[carId]/_components/car.comment.form";
import CarCommentCard from "@/app/(main)/cars/[carId]/_components/car.comment.card";

const BlogComment = async ({ blogId }: { blogId: string }) => {
  const session = await auth.api.getSession({
    headers: await headers(), // you need to pass the headers object.
  });

  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/comment/blog/${blogId}`, {
    cache: "no-store",
  });
  ...
  return (
    <div className="rounded-lg shadow-xs border border-slate-100 flex flex-col gap-4 p-4 bg-white">
      ...
      {session?.user && <CarCommentForm userId={session.user.id} blogId={blogId} />}
      {!session?.user && (
