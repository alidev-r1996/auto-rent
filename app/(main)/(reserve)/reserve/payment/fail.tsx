...
const FailPayment = ({ authority, orderId }) => {
  return (
    <div className="rounded-lg shadow-xs bg-radial items-center from-rose-400 to-rose-600 flex flex-col h-max gap-5 p-8 w-full mx-auto md:w-1/3 my-8">
      ...
      <div className="flex flex-col items-center gap-4 bg-slate-100 rounded-md p-3 mt-2">
        <div className="flex flex-col md:flex-rowflex-col md:flex-row items-center gap-1 md:gap-3">
          <p className="text-sm text-rose-600 text-center"> کد پیگیری :</p>
          <p className="text-sm text-rose-600 text-center font-bold">{PersianDigits(authority)}</p>
        </div>
        <div className="flex flex-col md:flex-row items-center gap-1 md:gap-3">
          <p className="text-sm text-rose-600 text-center"> شماره سفارش :</p>
          <p className="text-sm text-rose-600 text-center font-bold">{PersianDigits(orderId)}</p>
        </div>
      </div>
