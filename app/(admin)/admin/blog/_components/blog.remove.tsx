...
type RemoveBlogModalProps = {
  title: string;
  id: string;
  theme?: "blue" | "rose" | "teal" | "orange" | "dark" | "emerald";
};

const RemoveBlogModal: FC<RemoveBlogModalProps> = ({ title, theme = "dark", id }) => {
  const { isError, isPending, mutateAsync } = useRemoveBlog(id);
      ...
      <DialogContent dir="rtl">
        ...
        <DialogClose asChild className="flex items-center gap-1" dir="ltr">
          <div className="flex items-center gap mt-3">
            <Button onClick={() => mutateAsync()} variant={"destructive"}>
              حذف
            </Button>
