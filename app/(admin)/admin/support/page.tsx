...
const Reserve = () => {
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-4 flex flex-col gap-4 h-full">
      <UserHeader title="مدیریت گفتگوها " />
      {!data || (data.chats?.length == 0 && <div>هیچ داده ای وجود ندارد</div>)}
      {isError && <div>خطایی رخ داده است</div>}
      {data && data.chats?.length > 0 && (
        <ReserveTable chats={data.chats} info={data.info} theme={"dark"} />
      )}
